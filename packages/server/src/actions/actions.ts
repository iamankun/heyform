...
import { users_table, workspaces_table, projects_table, forms_table, submissions_table } from '../database/schema';

// Use the recommended pooled connection for most operations
const sql = neon(process.env.DATABASE_URL || process.env.POSTGRES_URL!);

// For operations requiring direct connection without pgbouncer
const sql_unpooled = neon(process.env.DATABASE_URL_UNPOOLED || process.env.POSTGRES_URL_NON_POOLING!);
...
export async function getFormAnalytics(form_id: string) {
  // Use unpooled connection for analytics queries
  const data = await sql_unpooled`
    SELECT 
